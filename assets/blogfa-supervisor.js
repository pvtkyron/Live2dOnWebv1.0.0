...
const PERF={mobile:matchMedia('(max-width:680px)').matches,reducedMotion:matchMedia('(prefers-reduced-motion:reduce)').matches,saveData:!!(navigator.connection&&navigator.connection.saveData)};
const H=window.__REV_SYSTEM_HEALTH__={version:VERSION,status:native?'native':safe?'safe-mode':'booting',widget:null,live2d:null,responsive:true,performance:PERF,attempts:[],failures:[],readyAt:null};
...
const deferLive2d=()=>new Promise(resolve=>{const delay=PERF.saveData?2200:PERF.mobile?900:120;const run=()=>bootLive2d().then(resolve);setTimeout(()=>{'requestIdleCallback'in window?requestIdleCallback(run,{timeout:PERF.mobile?2800:1600}):run();},delay);});
const boot=async()=>{try{await bootWidget();await deferLive2d();H.readyAt=Date.now();if(H.status!=='DEGRADED')paint('HEALTHY','全システム正常');}catch(e){fail('store',e);H.widget={status:'failed',message:String(e&&e.message||e)};paint('FAILSAFE','Blogfa本体はそのまま');}};
