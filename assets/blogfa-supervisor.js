...
const H=window.__REV_SYSTEM_HEALTH__={version:VERSION,status:native?'native':safe?'safe-mode':'booting',widget:null,live2d:null,attempts:[],failures:[],readyAt:null};
...
const fetchCode=async path=>{
    const tries=[
        {name:'jsdelivr',url:cdn(path)+'?rev='+Date.now()},
        {name:'github-api',url:api(path),headers:{Accept:'application/vnd.github.raw+json'}}
    ];
    let last;
    for(const x of tries){
        ...
        }catch(e){last=e;fail(path+':'+x.name,e);H.attempts.push({path,source:x.name,ok:false,at:Date.now()});}
    }
