...
if(menu&&nav){if(!nav.id)nav.id='site-nav';menu.setAttribute('aria-controls',nav.id);menu.setAttribute('aria-expanded','false');menu.setAttribute('aria-label','Open navigation');}
const closeMenu=()=>{if(!menu||!nav)return;nav.classList.remove('nav-active');menu.setAttribute('aria-expanded','false');menu.setAttribute('aria-label','Open navigation');menu.textContent='☰';};
if(menu&&nav){menu.addEventListener('click',()=>{const on=nav.classList.toggle('nav-active');menu.setAttribute('aria-expanded',String(on));menu.setAttribute('aria-label',on?'Close navigation':'Open navigation');menu.textContent=on?'×':'☰';});nav.addEventListener('click',e=>{if(e.target.closest('a'))closeMenu();});}
...
if(lang){const apply=v=>{document.documentElement.dataset.uiLang=v;lang.textContent=v==='fa'?'EN / FA':'FA / EN';lang.setAttribute('aria-pressed',String(v==='fa'));$$('.fa-copy').forEach(n=>n.style.display=v==='fa'?'block':'');};lang.addEventListener('click',()=>{const v=storage.get(langKey)==='fa'?'en':'fa';storage.set(langKey,v);apply(v);});apply(storage.get(langKey)||'en');}
const filter=$('.filterbar'),cards=$$('.product-card');
if(filter&&cards.length){const buttons=$$('button',filter);buttons.forEach(btn=>{btn.setAttribute('aria-pressed',String(btn.classList.contains('active')));btn.addEventListener('click',()=>{const key=btn.textContent.trim().toLowerCase();buttons.forEach(b=>{const on=b===btn;b.classList.toggle('active',on);b.setAttribute('aria-pressed',String(on));});cards.forEach(card=>{const type=($('.eyebrow',card)?.textContent||'').trim().toLowerCase(),hidden=key!=='all'&&!type.includes(key);card.classList.toggle('is-hidden',hidden);card.setAttribute('aria-hidden',String(hidden));});});});}
